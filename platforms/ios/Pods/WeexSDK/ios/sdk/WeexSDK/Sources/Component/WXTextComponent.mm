...
- (NSMutableAttributedString *)buildCTAttributeString
{
    ...
    CTFontRef ctFont = CTFontCreateWithName((__bridge CFStringRef)name,
                                           font.pointSize,
                                           NULL);
...
- (CTLineRef)buildTruncatedLineWithRuns:(CFArrayRef)runs lines:(NSMutableArray*)mutableLines path:(CGPathRef)cgPath
{
    ...
    if (lastLineRunCount > 0) {
        ...
        NSString *name =[uiFont fontName];
        if ([name isEqualToString:@".SFUI-Regular"]) {
            name = @"TimesNewRomanPSMT";
        }
        if (uiFont) {
            font = CTFontCreateWithName((__bridge CFStringRef)name, uiFont.pointSize, NULL);
        }
        if (font) {
