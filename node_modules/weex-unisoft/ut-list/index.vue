...
<template>
    <list class="list">
        <refresh v-if="$listeners['refresh']" class="refresh" @refresh="onrefresh" :display="refreshing ? 'show' : 'hide'">
            <slot name="refresh">
              <div class="indicator-div">
                ...
                <text class="indicator-text">{{refreshing?'加载中 ...':'下拉刷新'}}</text>
              </div>
            </slot>
        </refresh>
        <loading v-if="$listeners['loadmore'] && isRefresh" class="refresh" @loading="onloading" :display="loading ? 'show' : 'hide'">
            <slot name="loading">
              <div class="indicator-div">
        ...
        <cell :style="bottomStyle" v-if="!($listeners['loadmore'] && isRefresh)"/>
    </list>
</template>
...
export default {
  ...
  computed: {
      bottomStyle() {
        return {
          height: utils.isIPhoneX()?'160px':'140px'
        }
      },
      isRefresh() {
        if (this.$listeners['refresh']&&utils.isAndroid()) {
          return !this.noMore;
        }
        return true;
      }
